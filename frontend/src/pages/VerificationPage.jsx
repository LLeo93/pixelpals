...
import axios from 'axios'; // Usiamo axios direttamente qui, non axiosWithAuth, perché non siamo ancora autenticati
...
const VerificationPage = () => {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      const token = params.get('token'); // Ottieni il token dalla query string
      ...
      try {
        // Invia il token al tuo backend per la verifica
        // ASSICURATI CHE QUESTO URL CORRISPONDA AL TUO ENDPOINT BACKEND
    ...
    verifyEmail();
  }, [location.search]); // Rilancia useEffect se la query string cambia
    ...
    <>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-black to-blue-900 text-white font-inter relative overflow-hidden pt-16 pb-24">
        ...
        <div className="relative z-10 bg-gray-800 bg-opacity-90 shadow-2xl rounded-xl p-8 w-full max-w-md space-y-6 text-center border border-purple-700 transform transition-all duration-300 hover:scale-105">
          ...
          {isSuccess && (
            <button
              onClick={() => navigate('/')} // Reindirizza alla pagina di login
          ...
          {!isSuccess && (
            <button
              onClick={() => navigate('/')} // Reindirizza alla pagina di login anche in caso di errore
