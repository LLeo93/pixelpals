...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userRole !== 'ROLE_ADMIN') {
      console.warn(
        'Accesso negato: utente non è ADMIN. Reindirizzamento a /home.'
      );
      navigate('/home');
      return;
    }
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      console.log('Dati utenti ricevuti dal backend:', data);
      setUsers(data);
    } catch (err) {
  ...
  const handleSaveUser = async (formData) => {
    ...
    try {
      ...
      console.log('Payload inviato al backend:', dataToSend);
      ...
      console.error('Errore salvataggio utente (handleSaveUser):', err);
      throw err;
    }
  };
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-gray-700 rounded-lg shadow overflow-hidden border border-gray-600">
                ...
                <tbody className="text-gray-300 text-sm font-light">
                  {' '}
                  {/* Testo corpo più chiaro */}
                  {users.map((user) => (
                    <tr
                      key={user.id}
                      className="border-b border-gray-600 hover:bg-gray-600 transition duration-150" // Hover più scuro
                    >
                      ...
                      <td className="py-3 px-6 text-center">
                        <div className="flex items-center justify-center space-x-2">
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-blue-800 text-blue-300 hover:bg-blue-700 flex items-center justify-center transition duration-150" // Icone più scure
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-red-800 text-red-300 hover:bg-red-700 flex items-center justify-center transition duration-150" // Icone più scure
...
// Componente Modale per Aggiunta/Modifica Utente
const UserModal = ({ user, onSave, onClose, setModalError }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Errore nel salvataggio dal modale (UserModal):', err);
      console.log('Dettagli errore Axios:', err.response);
      if (err.response && err.response.data && err.response.data.message) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              value={formData.role}
              onChange={handleChange}
              className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500 text-gray-200" // Select scuro
            >
          ...
          <div className="flex items-center">
            <input
              ...
              onChange={handleChange}
              className="h-4 w-4 text-purple-400 focus:ring-purple-500 border-gray-600 rounded bg-gray-700" // Checkbox scura
            />
          ...
          <div className="flex justify-end space-x-4 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="bg-gray-700 hover:bg-gray-600 text-gray-200 font-semibold py-2 px-4 rounded-md transition duration-300" // Pulsante grigio scuro
            >
              Annulla
            </button>
            <button
              type="submit"
              className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-semibold py-2 px-4 rounded-md transition duration-300" // Pulsante gaming
            >
              Salva
            </button>
          </div>
...
// Componenti Input, Textarea e Alert aggiornati per il tema scuro
const Input = ({
  label,
  name,
  value,
  ...
  <div>
    ...
    <input
      ...
      required={required}
      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200" // Input scuro
    />
  </div>
);
...
const Textarea = ({ label, name, value, onChange }) => (
  <div>
    ...
    <textarea
      ...
      onChange={onChange}
      rows={4}
      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200" // Textarea scura
    />
  </div>
);
...
const Alert = ({ type, message }) => {
  const base =
    type === 'success'
      ? 'bg-green-900 border-green-700 text-green-300' // Alert verde scuro
      : 'bg-red-900 border-red-700 text-red-300'; // Alert rosso scuro
  return (
    <div
