...
const SetupProfilePage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Fetch all platforms
        const platformsRes = await axiosWithAuth.get('/platforms'); // Assumendo un endpoint /api/platforms
        setAllPlatforms(platformsRes.data);

        // Fetch current user data to pre-fill if they already have preferences
        const userRes = await axiosWithAuth.get('/auth/me');
        const userData = userRes.data;

        if (userData.preferredGames) {
          setPreferredGames(userData.preferredGames.map((g) => g.id)); // Store game IDs
          // Initialize skill levels from user data
          const initialSkills = {};
        ...
        if (userData.platforms) {
          setSelectedPlatforms(userData.platforms.map((p) => p.id)); // Store platform IDs
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      navigate('/home'); // Reindirizza alla homepage dopo il salvataggio
    } catch (err) {
          ...
          <div className="max-w-4xl mx-auto bg-gray-800 bg-opacity-90 p-8 rounded-xl shadow-2xl border border-blue-700 space-y-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex justify-center space-x-4 mt-8">
                ...
                <button
                  type="button"
                  onClick={() => navigate('/home')} // Permette di tornare alla home senza salvare
