...
const FriendsPage = () => {
  ...
  const [showConfirmModal, setShowConfirmModal] = useState(false); // Stato per la modale di conferma
  const [friendToRemove, setFriendToRemove] = useState(null); // Amico da rimuovere
  const currentUsername = localStorage.getItem('username');

  // Effetto per caricare i dati all'avvio e ogni volta che le dipendenze cambiano
  useEffect(() => {
    fetchFriendshipData();
  ...
  const fetchFriendshipData = async () => {
    ...
    try {
      ...
      // NUOVO: Rimuovi duplicati dalla lista amici per FriendsPage
      const uniqueFriends = Array.from(
        new Set(friendsRes.data.map((f) => f.id))
      ).map((id) => friendsRes.data.find((f) => f.id === id));
      setFriends(uniqueFriends);

      const pendingRes = await axiosWithAuth.get('/friends/pending');
      setPendingRequests(pendingRes.data); // <-- CORREZIONE QUI: da 'pending.data' a 'pendingRes.data'

      const sentRes = await axiosWithAuth.get('/friends/sent');
      setSentRequests(sentRes.data);

      // Dopo aver ricaricato i dati, ricalcola i risultati della ricerca
      // per assicurarti che i pulsanti siano aggiornati
      if (searchQuery.trim()) {
  ...
  // Funzione helper per determinare lo stato di amicizia di un utente
  ...
  const handleSendRequest = async (receiverUsername) => {
    ...
    setMessage('');
    try {
      // Aggiornamento ottimistico: aggiungi subito la richiesta inviata
      // con uno stato temporaneo, in attesa della conferma dal backend.
      ...
      // Dopo il successo, ricarica i dati per la conferma e la pulizia
      fetchFriendshipData();
    } catch (err) {
      ...
      // In caso di errore, ripristina lo stato precedente ricaricando i dati
      fetchFriendshipData();
    }
  };
  ...
  const handleAcceptRequest = async (requestId) => {
    ...
    try {
      ...
      fetchFriendshipData(); // Aggiorna tutte le liste dopo l'accettazione
    } catch (err) {
  ...
  const handleRejectRequest = async (requestId) => {
    ...
    try {
      ...
      fetchFriendshipData(); // Aggiorna tutte le liste dopo il rifiuto
    } catch (err) {
  ...
  const confirmRemoveFriend = async () => {
    ...
    try {
      ...
      fetchFriendshipData(); // Aggiorna tutte le liste dopo la rimozione
    } catch (err) {
      ...
      setFriendToRemove(null); // Resetta l'amico da rimuovere
    }
  };
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-black to-blue-900 text-white font-inter relative overflow-hidden">
        ...
        <main className="flex-grow p-4 pt-20 pb-24 relative z-10">
          <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {searchResults.map((user) => {
                      const status = getFriendshipStatus(user); // Ottieni lo stato di amicizia
                      return (
                        <div
                          key={user.id}
            ...
            {/* Sezione Richieste di Amicizia Ricevute */}
            ...
            {/* Sezione Richieste di Amicizia Inviate */}
